...
function argon_home_hide_config_vault($query){
    if (!is_home() || !$query->is_main_query()){
        return $query;
    }

    $knowledge_category = get_category_by_slug("config-vault");
    if (!$knowledge_category || is_wp_error($knowledge_category)){
        return $query;
    }

    $excluded_categories = $query->get("category__not_in");
    if (!is_array($excluded_categories)){
        $excluded_categories = empty($excluded_categories) ? array() : array($excluded_categories);
    }

    $excluded_categories = array_map("intval", $excluded_categories);
    $excluded_categories[] = (int) $knowledge_category->term_id;
    $excluded_categories = array_values(array_unique(array_filter($excluded_categories)));

    $query->set("category__not_in", $excluded_categories);
    return $query;
}
add_action("pre_get_posts", "argon_home_hide_config_vault", 20);

function argon_home_hide_config_vault_from_home_queries($query){
    if (is_admin() || !is_home()){
        return $query;
    }
...
add_action("pre_get_posts", "argon_home_hide_config_vault_from_home_queries", 21);
